...
# Direction: Hugging Face bucket -> local K2 evaluation artifacts.
...
Direction:
  HF bucket -> LOCAL K2 evaluation artifacts

Default sync pairs:
  hf://buckets/zhangdw/leo-benchmark/K2/evaluation/results_vllm      -> evaluation/results_vllm/
  hf://buckets/zhangdw/leo-benchmark/K2/evaluation/results_vllm_chat -> evaluation/results_vllm_chat/
...
Examples:
  ...
  hfsync/remote_to_local.sh --artifact results_vllm_chat --dry-run
  hfsync/remote_to_local.sh --new-only
  hfsync/remote_to_local.sh --artifact legacy_results,pickle
USAGE
}
